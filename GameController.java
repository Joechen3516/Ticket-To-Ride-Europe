...
public class GameController {
	...
	public void HandleAction(ActionEvents x, TrainColor trainColor, int roadNum) {
		if(roadNum == -2 && x == ActionEvents.Cancel) {
			selectedTrainCards = new ArrayList<TrainCard>();
			currentCities.clear();
			guiState = GuiState.nothing;
			cardTurn = 0;
			...
			roadNum = 0;
		ArrayList<Road> roads = europe.roadSearch(europe.citySearch(currentCities.get(0)), europe.citySearch(currentCities.get(1)));
		Road road = roads.get(roadNum);
		if(x == ActionEvents.addedTrainCard) {
